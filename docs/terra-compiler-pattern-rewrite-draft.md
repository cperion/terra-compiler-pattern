...
> Built section by section so the original document is not overwritten prematurely.
...
### The sentence this section should leave the reader with

If this section succeeds, the reader should come away with one corrected mental model:
...
### The sentence this section should leave behind

If the reader leaves this section with one sentence in mind, it should be this:
...
If this section leaves one durable idea, it should be this:
...
If the reader leaves this section with one durable sentence, it should be this:
...
1. design the domain and phases backend-neutrally
2. implement the pure layer once
3. target LuaJIT first on JIT-native platforms
4. benchmark the important leaf families and compositions
5. opt into Terra where explicit native power is worth the extra cost

This policy matches both the architecture and the current empirical results.
...
2. realize terminal families on LuaJIT by default where appropriate
3. benchmark and inspect the workloads that matter
4. move a terminal family or subsystem to Terra when explicit native control buys enough
5. keep the source model and phase story shared whenever possible
