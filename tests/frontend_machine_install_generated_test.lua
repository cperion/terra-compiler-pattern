...
local function test_install_generated_attaches_runtime_closures()
    ...
    assert(#token_spec.tokens == 6)
    assert(token_spec.tokens[1].kind == "ModuleKw")
    assert(token_spec.tokens[1].span.start_byte == 5)
    assert(token_spec.tokens[2].kind == "Ident")
    assert(token_spec.tokens[2].text == "Demo")
    assert(token_spec.tokens[3].kind == "LBrace")
    assert(token_spec.tokens[4].kind == "Ident")
    assert(token_spec.tokens[4].text == "Inner")
    assert(token_spec.tokens[5].kind == "RBrace")
    assert(token_spec.tokens[6].kind == "Eof")

    local token_spec2 = target_ctx.TargetText.Spec.tokenize({ text = "modulex" })
    assert(#token_spec2.tokens == 2)
    assert(token_spec2.tokens[1].kind == "Ident")
    assert(token_spec2.tokens[1].text == "modulex")

    local ok1, err1 = pcall(function()
        return target_ctx.TargetText.Spec.tokenize({ text = "@" })
    end)
    assert(not ok1)
    assert(tostring(err1):match("generated tokenize closure invalid token"))
