...
local function test_define_machine_builds_runtime_shape()
    ...
    assert(#machine.tokenize.fixed_dispatches == 3)
    assert(#machine.tokenize.ident_dispatches == 1)
    assert(machine.tokenize.eof_ctor_id == 5)
    assert(#machine.parse.result_ctors == 1)
    assert(#machine.parse.rules == 1)
    assert(machine.parse.rules[1].kind == "SeqRule")
    assert(machine.parse.rules[1].result.kind == "ReturnCtor")
