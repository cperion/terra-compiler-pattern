...
local function image_intrinsic(T, _assets, _image)
    -- There is not yet a dedicated image-metadata phase/catalog carrying
    -- intrinsic pixel size, so ui2 starts with the same simple placeholder
    -- policy as the earlier example pipeline.
    --
    -- This is intentionally isolated here so we can later replace it with a
    -- better explicit asset-derived intrinsic phase without disturbing the rest
    -- of the solver input language.
