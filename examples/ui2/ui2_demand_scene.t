...
local function text_layout_for(T, text)
    return T.UiCore.TextLayout(text.wrap, text.overflow, text.align, text.line_limit)
end

local function shape_text(T, assets, text, bounds)
    local raw = RawText.shape(
        nil,
        Assets.font_path(require_catalog(assets, "UiDemand.Scene:solve"), text.font),
        text.value,
        text_style_for(T, text),
        text_layout_for(T, text),
        bounds
    )

    local lines = {}
    for _, line in ipairs(raw.lines) do
        local runs = {}
        for _, run in ipairs(line.runs) do
            local glyphs = {}
            for _, glyph in ipairs(run.glyphs) do
                glyphs[#glyphs + 1] = T.UiSolved.GlyphPlacement(
                    glyph.glyph_id,
                    glyph.origin.x,
                    glyph.origin.y
                )
            ...
            runs[#runs + 1] = T.UiSolved.ShapedRun(
                text.font,
                run.size_px,
                text.color,
                L(glyphs)
        ...
        lines[#lines + 1] = T.UiSolved.ShapedLine(
            line.baseline_y,
            line.ink_bounds.w,
            L(runs)
        )
    ...
    return T.UiSolved.ShapedText(
        text.value,
        rect(T, raw.bounds.x, raw.bounds.y, raw.bounds.w, raw.bounds.h),
        text.wrap,
        text.align,
        L(lines)
    )
...
local function draw_atoms_for(T, assets, node, geometry, visual_state)
    ...
    U.match(node.demand, {
        ...
        TextDemand = function(v)
            draws[#draws + 1] = T.UiSolved.TextDraw(
                shape_text(T, assets, v.text, geometry.content_box),
                visual_state
            )
        end,
        ImageDemand = function(v)
...
return function(T)
    ...
    --   final solved text output still needs font-backed shaping. That is a
    --   compiler concern driven by explicit resources, so it remains a normal
    --   boundary argument instead of hidden ambient state.
