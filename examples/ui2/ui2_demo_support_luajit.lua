...
return function(T, Backend)
    ...
    local custom_handler = function(rt, payload)
        GL.glDisable(C.GL_TEXTURE_2D)
        GL.glColor4d(0.22, 0.95, 0.58, 0.25 * rt.opacity)
        GL.glBegin(C.GL_QUADS)
        GL.glVertex2d(20.0 + payload, 18.0 + payload)
        GL.glVertex2d(72.0 + payload, 18.0 + payload)
        GL.glVertex2d(72.0 + payload, 38.0 + payload)
        GL.glVertex2d(20.0 + payload, 38.0 + payload)
        GL.glEnd()
    end
