...
function Decode.install(T)
    T.TaskView.Screen.decode = U.transition(function(screen, intent)
        return U.match(intent, {
            Command = function(v)
                return T.TaskDecode.Result(L { event_for_command(T, Codec.decode(T, v.command)) })
            end,
            Toggle = function(v)
                return T.TaskDecode.Result(v.command and L { event_for_command(T, Codec.decode(T, v.command)) } or L())
            end,
            Scroll = function(_)
