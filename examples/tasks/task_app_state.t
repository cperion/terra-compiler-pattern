...
return function(T)
    ...
    local function tag_chips(workspace, refs)
        ...
        local content = project and (function()
            ...
            return T.TaskView.ProjectScreen(
                ...
                L(F.iter(visible_tasks):map(function(task)
                    return T.TaskView.TaskRow(task.id, task.title, note_preview(task.notes), task.status, task.priority, tag_chips(state.workspace, task.tags), selected_task == task.id)
                end):totable()),
                detail_task and T.TaskView.TaskDetail(T.TaskView.TaskCard(detail_task.id, detail_task.title, detail_task.notes, detail_task.status, detail_task.priority, tag_chips(state.workspace, detail_task.tags))) or T.TaskView.NoDetail()
            )
