...
local function L(xs)
    ...
    local function status_filters(workspace, project, filter)
        local statuses = {
            T.TaskCore.Todo(),
    ...
    local function tag_filters(workspace, project, filter)
    ...
    local function tag_chips(workspace, refs, scope)
        return L(F.iter(refs):enumerate():map(function(i, ref)
            local tag = find_tag(workspace, ref)
            return T.TaskView.TagChip(scope * 1000 + i, tag.id, tag.name, tag.color)
        end):totable())
    end

    local function draft_form(workspace, draft, base_id)
        local statuses = {
            T.TaskCore.Todo(),
            T.TaskCore.InProgress(),
            T.TaskCore.Blocked(),
    ...
    local function next_task_id(workspace)
        local max_id = F.iter(workspace.projects):map(function(project)
            return F.iter(project.tasks):map(function(task)
    ...
    local function with_projects(workspace, projects)
        return T.TaskDoc.Workspace(workspace.version, workspace.name, L(projects), workspace.tags)
    end

    local function update_project_tasks(workspace, project_ref, mapper)
        return with_projects(workspace, F.iter(workspace.projects):map(function(project)
    ...
    T.TaskApp.State.project_view = U.transition(function(state)
        ...
        local sidebar = T.TaskView.Sidebar(
            ...
            T.TaskView.FilterPanel(
                ...
                project and status_filters(state.workspace, project, state.session.filter) or L(),
                project and tag_filters(state.workspace, project, state.session.filter) or L(),
        ...
        local content = project and (function()
            ...
            return T.TaskView.ProjectScreen(
                ...
                L(F.iter(visible_tasks):map(function(task)
                    return T.TaskView.TaskRow(task.id, task.title, note_preview(task.notes), task.status, task.priority, tag_chips(state.workspace, task.tags, 10000 + task.id.value), selected_task == task.id)
                end):totable()),
                detail_task and T.TaskView.TaskDetail(T.TaskView.TaskCard(detail_task.id, detail_task.title, detail_task.notes, detail_task.status, detail_task.priority, tag_chips(state.workspace, detail_task.tags, 20000 + detail_task.id.value))) or T.TaskView.NoDetail()
            )
        ...
        local overlays = U.match(state.session.editor, {
            ...
            CreatingTask = function(v)
                return L { T.TaskView.TaskEditorOverlay(T.TaskView.CreateTaskEditor(v.project, draft_form(state.workspace, v.draft, v.project.value))) }
            end,
            EditingTask = function(v)
                return L { T.TaskView.TaskEditorOverlay(T.TaskView.EditTaskEditor(v.task, draft_form(state.workspace, v.draft, 5000 + v.task.value))) }
            end,
        ...
        return U.match(event, {
            ...
            SubmitEditor = function()
                return U.match(state.session.editor, {
                    ...
                    ConfirmingDelete = function() return state end,
                    CreatingTask = function(ed)
                        ...
                        local workspace = update_project_tasks(state.workspace, ed.project, function(tasks)
                            local out = F.iter(tasks):totable()
                            out[#out + 1] = new_task
                            return out
                        end)
                    ...
                    EditingTask = function(ed)
                        local workspace = update_project_tasks(state.workspace, task_project_ref(state.workspace, ed.task), function(tasks)
            ...
            ConfirmDelete = function()
                return U.match(state.session.editor, {
                    ConfirmingDelete = function(ed)
                        ...
                        local workspace = update_project_tasks(state.workspace, project_ref, function(tasks)
            ...
            SetTaskStatus = function(v)
                local project_ref = task_project_ref(state.workspace, v.task)
                return U.with(state, { workspace = update_project_tasks(state.workspace, project_ref, function(tasks)
                    return F.iter(tasks):map(function(task)
