...
local function materialize(state, assets, viewport)
    ...
    return {
        ...
        times = {
            ...
            bind = t3 - t2,
            flatten = t4 - t3,
            prepare_demands = t5 - t4,
            solve = t6 - t5,
            plan = t7 - t6,
            specialize_kernel = t8 - t7,
            define_machine = t9 - t8,
            compile = t10 - t9,
            materialize = t11 - t10,
            total = t11 - t0,
        }
...
local function print_shape_counts(result)
    print((
        "  nodes screen=%d document=%d plan hits=%d focus=%d key=%d scroll=%d edit=%d accessibility=%d"
    ):format(
        node_size(result.screen),
        node_size(result.document),
        #(result.plan.hits or {}),
        #(result.plan.focus_chain or {}),
        #(result.plan.key_routes or {}),
        #(result.plan.scroll_hosts or {}),
        #(result.plan.edit_hosts or {}),
        #(result.plan.accessibility or {})
    ))
end
...
local function print_phase_stats(prefix, phase_samples)
    ...
    print(("  %s flatten           %s"):format(prefix, fmt_stats(phase_samples.flatten)))
    print(("  %s prepare_demands   %s"):format(prefix, fmt_stats(phase_samples.prepare_demands)))
    print(("  %s solve             %s"):format(prefix, fmt_stats(phase_samples.solve)))
    print(("  %s plan              %s"):format(prefix, fmt_stats(phase_samples.plan)))
    print(("  %s specialize_kernel %s"):format(prefix, fmt_stats(phase_samples.specialize_kernel)))
    print(("  %s define_machine    %s"):format(prefix, fmt_stats(phase_samples.define_machine)))
    print(("  %s compile           %s"):format(prefix, fmt_stats(phase_samples.compile)))
    print(("  %s materialize       %s"):format(prefix, fmt_stats(phase_samples.materialize)))
...
local function benchmark_full_rebuild(task_count, scenario, assets, viewport, warmup, iters, project_count, tag_count)
    ...
    local summary = {
        ...
        route_hits = #(first_result.plan.hits or {}),
        route_focus = #(first_result.plan.focus_chain or {}),
        route_scroll = #(first_result.plan.scroll_hosts or {}),
        route_key = #(first_result.plan.key_routes or {}),
        route_edit = #(first_result.plan.edit_hosts or {}),
        route_accessibility = #(first_result.plan.accessibility or {}),
    }
...
local function benchmark_steady_state(task_count, scenario, assets, viewport, warmup, iters, project_count, tag_count)
    ...
    for i = 1, warmup do
        ...
        local _ = T.UiSession.State.initial(viewport):apply(plan, input)
    end

    local ui_session = T.UiSession.State.initial(viewport)

    local apply_samples = {}
    for i = 1, iters do
        local input = (scenario == "modal" and T.UiInput.TextEntered("x")) or T.UiInput.PointerMoved(T.UiCore.Point(32 + i, 48 + i))
        local t0 = now_ns()
        ui_session = ui_session:apply(plan, input)
        local t1 = now_ns()
        push(apply_samples, t1 - t0)
    end
...
local function main()
    ...
    local runtime = Backend.init_window("tasks vs clay bench", viewport.w, viewport.h)
    ...
    print("ui benchmark: TaskApp.State -> TaskView.Screen -> UiDecl.Document -> UiBound.Document -> UiFlat.Scene -> UiDemand.Scene -> UiSolved.Scene -> UiPlan.Scene -> UiKernel.Render -> UiMachine.Render -> Unit")
