-- GrammarSource.Grammar boundary
-- Installs: compile() terminal (closure tree) + builder DSL
...
-- ═══════════════════════════════════════════════════════════════
-- Closure-tree compiler
-- ═══════════════════════════════════════════════════════════════

local compile_pattern

compile_pattern = function(pattern, rules_map, deferred, U)
    ...
    return U.match(pattern, {
        ...
        Sequence = function(p)
            ...
            for i = 1, #p.patterns do
                parts[i] = compile_pattern(p.patterns[i], rules_map, deferred, U)
            end
            local n = #parts
            if n == 0 then
                return function(buf, pos, limit, caps) return pos end
        ...
        Choice = function(p)
            ...
            for i = 1, #p.alternatives do
                alts[i] = compile_pattern(p.alternatives[i], rules_map, deferred, U)
            end
            local n = #alts
            if n == 0 then
        ...
        Repeat = function(p)
            local inner = compile_pattern(p.pattern, rules_map, deferred, U)
...
local function compile_grammar(grammar, U)
    local rules_map = {}
    local deferred = {}
    ...
    for i = 1, #grammar.rules do
        ...
        rules_map[name] = compile_pattern(rule.body, rules_map, deferred, U)
    end
...
local function install_builder(T)
    ...
    function P.seq(...)
        ...
        return G.Sequence(args)
    end

    function P.alt(...)
        local args = { ... }
        if #args == 1 then return args[1] end
        return G.Choice(args)
    end
...
return function(T, U, P)
    T.GrammarSource.Grammar.compile = U.terminal(function(grammar)
        return compile_grammar(grammar, U)
    end)
    install_builder(T)
end
